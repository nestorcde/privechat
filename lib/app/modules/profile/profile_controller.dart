...
class ProfileController extends GetxController {
  ...
  void guardarDatos() {
    ...
    repository
        .updateProfile(nombreController.value.text,
            telefonoController.value.text, usuarioObs.value)
        .then((value) {
  ...
  void uploadImage(ImageSource imageSource) async {
    try {
      ...
      if (pickedFile != null) {
        //var response = await
        repository
            .uploadFile(file!, usuarioObs.value, pickedFile.name)
            .then((response) async {
          ...
          if (/*response['statusCode']*/ response['ok']) {
            ...
            usuarioObs.value =
                updateProfileResponseFromJson(jsonEncode(response)).usuario;
            returnImage1();
            update();
          ...
          // }
            ...
            Get.snackbar('Fallido',
                'Error: ${response['msg']}' /*response['statusCode']*/,
                margin: const EdgeInsets.only(top: 5, left: 10, right: 10));
          }
        });
      } else {
  ...
  returnImage() =>
      profileImage = NetworkImage(url + usuarioObs.value.imgProfile!).obs;
  returnImage1() =>
      profileImage.value = NetworkImage(url + usuarioObs.value.imgProfile!);
}
